...
if docker volume inspect "${compose_project_name}_data" &>/dev/null; then
  echo "" && echo "=== Deleting forger-data volume ===" && echo ""
  docker volume rm "${compose_project_name}_data" || { echo "Error: could not delete ${compose_project_name}_data for some reason. Can not proceed any further.  Exiting ..."; exit 1; }
else
  echo "" && echo "=== ${compose_project_name}_data volume does not exist. Nothing to delete ... ===" && echo ""
fi

if docker volume inspect "${compose_project_name}_snark-keys" &>/dev/null; then
  echo "" && echo "=== Deleting snark-keys volume ===" && echo ""
  docker volume rm "${compose_project_name}_snark-keys" || { echo "Error: could not delete ${compose_project_name}_snark-keys for some reason. Can not proceed any further.  Exiting ..."; exit 1; }
else
  echo "" && echo "=== ${compose_project_name}_snark-keys volume does not exist. Nothing to delete ... ===" && echo ""
fi
